...
import object_detection_tflite as od


class Cam:
    saved_img_filename=""
    min_conf = 0.10
    max_boxes = 2   # max box count, max number of colonies to detect
    ...
    x=10; y=20; width=412; height=412;
    
    # zx was 0.5,  made smaller to shift initial window position left
    zx=0.30; zy=0.480; zw=0.440; zh=0.440;
    ...
    def menu(self):
        ...
        print('a autofocus')
        print('r object detection on saved image')
    ...
    def save_pic(self, filename):
        try:
          ...
          self.saved_img_filename=filename
        finally:
          pass
    ...
    def update(self, arducam_vcm, keyVal):
        ...
        if keyVal == self.location_5_holder:
            self.zx=0.300; self.zy=0.480; self.zw=0.440; self.zh=0.440;
            self.camera.zoom=(self.zx, self.zy, self.zw, self.zh)
...
try:
    ...
    while True:
        ...
        if keyVal == ord('?'):
            ...
            print(">> ", myCam.saved_img_filename)
            if os.path.isfile(myCam.saved_img_filename):
                TEMP_FILE = myCam.saved_img_filename
            else:
                TEMP_FILE ='images-test/carotene-01-600-600.jpg'    # just testing
            
            print("\nObject Detection: %s,  min_conf %0.2f, max_boxes %d\n" % \
                  (TEMP_FILE, myCam.min_conf, myCam.max_boxes))
            

            od.run_od(TEMP_FILE, myCam.min_conf, myCam.max_boxes, keyboard)
